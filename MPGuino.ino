...
const uint8_t mbuttonBit = 		0b00001000; // GPIO A bit 3, up button
const uint8_t rbuttonBit = 		0b00000010; // GPIO A bit 1, right button
const uint8_t abuttonBit = 		0b00000001; // GPIO A bit 0, select button
...
#endif // useAdafruitRGBLCDmodule
#ifdef useTWIbuttons
volatile uint8_t buttonReadCount;
volatile uint8_t buttonReadState;
volatile uint8_t thisTWIbuttonPressed;
volatile uint8_t lastTWIbuttonPressed;
...
const unsigned int vssResetDelay = (int)(500ul * t2CyclesPerSecond / 256000ul); // VSS pulse timeout is 500 milliseconds
const unsigned int cursorDelay = (int)(500ul * t2CyclesPerSecond / 256000ul); // display cursor toggle period is 500 milliseconds

const unsigned int holdDelay = (int)(2000ul * t2CyclesPerSecond / 256000ul); // 2 second delay
const unsigned int delay1500ms = (int)(1500ul * t2CyclesPerSecond / 256000ul); // 1.5 second delay
const unsigned int delay0005ms = (int)(5ul * t2CyclesPerSecond / 256000ul); // 5 millisecond delay
...
char * format64(const uint8_t * prgmPtr, char * strBuffer, uint8_t decimalPlaces);
...
void doCursorUpdateSetting(void); /* Setting selector section */
void doSettingEditDisplay(void);
void doGoSettingsEdit(void);
void doCursorUpdateParam(void); /* Individual parameter editor section */
...
void doParamReadMaxValue(void);
void doParamReadMinValue(void);
void doParamRevert(void);
void doParamReadEEPROM(uint8_t parameterPtr);
void doParamFormatInputNumber(unsigned long v);
void doParamChangeDigit(void);
void doParamRemoveLeadZero(void);
void doParamWriteEEPROM(void);
...
#endif // useBenchMark
#endif // useCPUreading
#ifdef useEEPROMviewer // base EEPROM viewer support section
...
uint8_t eepromWriteVal(uint8_t eePtr, unsigned long val);
unsigned long eepromReadVal(uint8_t eePtr);
unsigned int eepromGetAddress(uint8_t eePtr);
...
const uint8_t accelTestClearFlags =		accelTestInProgress | accelTestMeasurementFlags | accelTestCancelled | accelTestFinished;
...
const uint8_t tcTakeAnalogSample =	0b00001000;
const uint8_t tcBlockTWIbutton =	0b00000100;
...
const uint8_t tsTWIbuttonActive =	0b00000100;
...
#define nextAllowedValue pAddressDragDistance + byteSize(pSizeDragDistance)
#endif // useDragRaceFunction
...
#define nextAllowedValue pDragDistanceIdx + 1
#endif // useDragRaceFunction
...
static const buttonList bpListMenu[] PROGMEM = {
	{btnShortPressRL,	doReturnToMainScreen}
	,{btnShortPressC,	menu::select}
	,{btnLongPressRL,	doReturnToMainScreen}
#ifdef useCPUreading
	,{btnLongPressC,	doShowCPU}
#endif // useCPUreading
	,{buttonsUp,		noSupport}
...
static const buttonList bpListParam[] PROGMEM = {
	{btnShortPressRC,	doParamFindRight}
	,{btnShortPressRL,	doParamExit}
	,{btnShortPressCL,	doParamFindLeft}
	,{btnShortPressC,	doParamChangeDigit}
	,{btnLongPressRC,	doParamReadMinValue}
	,{btnLongPressRL,	doParamRevert}
	,{btnLongPressCL,	doParamReadMaxValue}
	,{btnLongPressC,	doParamWriteEEPROM}
	,{buttonsUp,		noSupport}
};
...
static const buttonList bpListDragRace[] PROGMEM = {
	{btnShortPressC,	doNextBright}
	,{btnLongPressC,	accelerationTest::goTrigger}
	,{btnLongPressRL,	doReturnToMainScreen}
#ifdef useDebugReadings
#ifdef useTWIbuttons
	,{btnLongPress2,	goStepDebug}
...
static const buttonList bpListCoastdown[] PROGMEM = {
	{btnShortPressC,	doNextBright}
	,{btnLongPressC,	coastdown::goTrigger}
	,{btnLongPressRL,	doReturnToMainScreen}
#ifdef useDebugReadings
#ifdef useTWIbuttons
	,{btnLongPress2,	goStepDebug}
...
static const modeInformation screenParameters[(unsigned int)(totalMenuSize)] PROGMEM = {
	...
	,{settingScreenIdx | 0x80,		1,			eePtrSettingsEnd - 1,			doSettingEditDisplay,		doCursorUpdateSetting,		bpListSetting}
	,{paramScreenIdx | 0x80,		1,			12,					doParamEditDisplay,		doCursorUpdateParam,		bpListParam}
#ifdef useSavedTrips
...
#endif // useScreenEditor
#ifdef useEEPROMviewer
	,{eepromViewIdx | 0x80,			1,			eePtrEnd,				doEEPROMviewDisplay,		doNothing,			bpListEEPROMview}
#endif // useEEPROMviewer
#ifdef useDragRaceFunction
...
namespace text // text string output prototype
{
	...
	void charOut(outputDevice &dev, uint8_t chr);
	...
	void blinkStringOut(outputDevice &dev, char * str, uint8_t condition);
	void blinkStringOut(outputDevice &dev, const char * str, uint8_t condition);
	void blinkStringOut(outputDevice &dev, const char * str, uint8_t strIdx, uint8_t condition);
...
char mBuff1[17]; // used by formatDecimal(), doFormat()
...
#endif // useAnalogRead
#ifdef useDragRaceFunction
const uint8_t internalCancelDragRace =	0b00000010;
#endif // useDragRaceFunction
#ifdef useCoastDownCalculator
const uint8_t internalCancelCDT =	0b00000001;
#endif // useCoastDownCalculator
...
ISR( TIMER0_OVF_vect ) // system timer interrupt handler
{
...
#ifdef useDragRaceFunction
				internalFlags |= internalCancelDragRace;
#endif // useDragRaceFunction
...
#ifdef useDragRaceFunction
				internalFlags |= internalCancelDragRace; // acceleration test will cancel if vehicle is idling
#endif // useDragRaceFunction
#ifdef useCoastDownCalculator
...
#endif // useCoastDownCalculator
#ifdef useDragRaceFunction
	if (internalFlags & internalCancelDragRace)
	{
		...
		if (accelerationFlags & accelTestInProgress) // if acceleration test has started
		{
			...
			accelerationFlags |= (accelTestCancelled | accelTestFinished); // signal that drag function is cancelled
		...
		switch (buttonReadState)
		{
...
#endif // useAdafruitRGBLCDmodule
					buttonReadState++; // advance to waiting for this request to finish
			...
			case 1:
				if ((twiStatusFlags & twiTxRxStatus) == 0) // wait for TWI button pin register address write to complete
				{
					...
					buttonReadState++; // advance to waiting for button read to complete
			...
			case 2:
				if ((twiStatusFlags & twiTxRxStatus) == 0) // wait for TWI button read to complete
				{
					...
					if (twiStatusFlags & twiErrorFlag) buttonReadState = 255; // if an error occurred during read, exit
					else
					{
...
#endif // useAdafruitRGBLCDmodule
						buttonReadState++; // advance to debouncing initialization
			...
			case 3:
				thisTWIbuttonPressed = (thisTWIbuttonPressed & btnShortPress21RCL);
	...
	else
	{
		...
		else
		{
			...
			if (timerCommand & tcBlockTWIbutton) timerStatus &= ~(tsTWIbuttonActive);
			else
			{

				timerStatus |= tsTWIbuttonActive;
				...
				buttonReadState = 0; // initialize TWI button read state machine
	...
	else
	{
		...
		timerCommand |= (tcTakeAnalogSample | tcUpdateDisplay); // signal to main program that a sampling should occur, and to update display
...
ISR( INJ_CLOSE_vect ) // injector closing event handler
{
	...
	if (injOpenCycleLength)
	{
		...
		if (accelerationFlags & accelTestInProgress)
		{
			...
			tripArray[(unsigned int)(dragDistanceIdx)].collectedData[(unsigned int)(rvInjPulseIdx)]++; // update the distance acceleration injector pulse count
			tripArray[(unsigned int)(dragDistanceIdx)].add64s(rvInjOpenCycleIdx, injOpenCycleLength); // add to distance acceleration fuel injector open cycle accumulator
...
#ifdef useDragRaceFunction
	if (accelerationFlags & accelTestInProgress) tripArray[(unsigned int)(dragDistanceIdx)].add64s(rvInjCycleIdx, totalInjCycleLength); // add to distance acceleration fuel injector total cycle accumulator
...
#endif // useCoastDownCalculator
#ifdef useDragRaceFunction
		if (accelerationFlags & accelTestInProgress) // if accel test has been requested or is active
		{

			if (accelerationFlags & accelTestTriggered) // if drag function is triggered
			{
			...
			tripArray[(unsigned int)(dragDistanceIdx)].collectedData[(unsigned int)(rvVSSpulseIdx)]++; // update the accel test distance measurement VSS pulse count
			tripArray[(unsigned int)(dragDistanceIdx)].add64s(rvVSScycleIdx, cycleLength); // update the accel test distance measurement VSS cycle accumulator
			...
			if ((accelerationFlags & accelTestHalfSpeed) && (cycleLength < volatileVariables[(unsigned int)(vAccelHalfPeriodValueIdx)])) // if instantaneous speed is greater than drag function speed
			{
				...
				tripArray[(unsigned int)(dragHalfSpeedIdx)].transfer(tripArray[(unsigned int)(dragDistanceIdx)]);
			...
			if ((accelerationFlags & accelTestFullSpeed) && (cycleLength < volatileVariables[(unsigned int)(vAccelFullPeriodValueIdx)])) // if instantaneous speed is greater than drag function speed
			{
				...
				// copy drag raw trip variable to drag full speed trip variable
				tripArray[(unsigned int)(dragFullSpeedIdx)].transfer(tripArray[(unsigned int)(dragDistanceIdx)]);
...
#ifdef useTWILCD
#ifdef useTWIbuttons
	timerCommand |= tcBlockTWIbutton; // disable TWI button reading as it interferes with TWI LCD output
	while (timerStatus & tsTWIbuttonActive) idleProcess(); // wait for any in-progress TWI button reads to finish
...
#ifdef useTWILCD
#ifdef useTWIbuttons
	timerCommand &= ~(tcBlockTWIbutton); // re-enable TWI button reading
...
#ifdef useLegacyLCD
	setContrast(eepromReadVal(pContrastIdx));
#endif // useLegacyLCD
...
#ifdef useTWILCD
#ifdef useAdafruitRGBLCDmodule
	if (idx) setRGBcolor(eepromReadVal(pLCDcolorIdx)); // turn on LCD backlight
...
uint8_t LCD::bufferDevice(uint8_t cmd, uint8_t data)
{
	...
	switch (cmd)
	{
		...
		case (rbvOpenOutput):
#ifdef useTWILCD
			TWI::initWrite(lcdAddress, TWI_STOP, TWI_INTERRUPT); // open TWI as master transmitter, with interrupt flag set
#ifdef useAdafruitRGBLCDmodule
			TWI::writeByte(MCP23017_GPIOx | MCP23017_PORTA); // specify bank A output latch register address
#endif // useAdafruitRGBLCDmodule
#endif // useTWILCD
...
uint8_t tripVar::load(uint8_t tripPos)
{
	...
	uint8_t b = (uint8_t)(eepromReadVal(t + tripSlotSignaturePtr));
	...
	if (b == guinosig)
	{
		...
		for (uint8_t x = 0; x < tripListPtrLength; x++) collectedData[(unsigned int)(x)] = eepromReadVal(++t);
...
#ifdef useSoftwareClock
	eepromWriteVal(t++, convertTime((uint32_t * )(&clockCycles)));
#else
	eepromWriteVal(t++, convertTime((uint32_t *)(&systemCycles)));
#endif

	for (uint8_t x = 0; x < tripListPtrLength; x++) eepromWriteVal(t++, collectedData[(unsigned int)(x)]);
...
#endif
uint64_t tmp1 = 0;	// general purpose
uint64_t tmp2 = 0;	// output value / general purpose
uint64_t tmp3 = 0;	// general purpose / temporary storage
uint64_t tmp4 = 0;	// used in multiply, divide operations
uint64_t tmp5 = 0;	// used in multiply, divide operations
...
unsigned long SWEET64(const uint8_t * sched, uint8_t tripIdx)
{
	...
	while (loopFlag)
	{
		...
		if (instr & 0x40)
		{
			...
			tu1 = tempPtr[(unsigned int)((operand >> 4) & 0x07)];
			tu2 = tempPtr[(unsigned int)(operand & 0x07)];
		...
		switch (instr)
		{
			...
			case instrLdEEPROMindexed:
				operand += tripIdx;
				init64(tu2, eepromReadVal(operand));
				break;
			...
			case instrStEEPROMindirect:
				operand = pgm_read_byte(&convIdx[(unsigned int)(tripIdx)]);
			case instrStEEPROM:
				eepromWriteVal(operand, tu2->ul[0]);
				break;
			...
			case instrDoBCDadjust:
				...
				else
				{
					...
					tripIdx = 3;
					tu2->u8[0] = tempPtr[1]->u8[0]; // workaround to support 10 decimal digits / 5 BCD bytes
		...
		if (SWEET64processorFlags & SWEET64traceFlag)
		{
			...
			for (uint8_t x = 0;x < 6; x++)
			{
				...
				text::hexDWordOut(devDebugSerial, tempPtr[(unsigned int)(x)]->ul[1]);
				text::charOut(devDebugSerial, 32);
				text::hexDWordOut(devDebugSerial, tempPtr[(unsigned int)(x)]->ul[0]);
				text::charOut(devDebugSerial, 13);
				text::charOut(devDebugSerial, 13);
...
#endif
#ifndef useSWEET64multDiv
void mul64(union union_64 * an, union union_64 * ann)			// multiplier in an, multiplicand in ann, result to an
{

	union union_64 * multiplier = tempPtr[3];			// define multiplier term as register 4
	union union_64 * multiplicand = tempPtr[4];			// define multiplicand term as register 5
...
void div64(union union_64 * an, union union_64 * ann)			// dividend in an, divisor in ann
{
	...
	union union_64 * divisor = tempPtr[3];
	uint8_t x;
	uint8_t y;
	uint8_t z;
...
void text::charOut(outputDevice &dev, uint8_t chr)
{
...
void text::stringOut(outputDevice &dev, const char * str)
{
	...
	dev.controlFlags |= odvFlagEnableOutput; // enable text output, disable blanking output
	while (0 != (chr = pgm_read_byte(str++))) charOut(dev, chr);
...
void text::blinkStringOut(outputDevice &dev, char * str, uint8_t condition)
{
...
void text::blinkStringOut(outputDevice &dev, const char * str, uint8_t condition)
{
...
void text::blinkStringOut(outputDevice &dev, const char * str, uint8_t strIdx, uint8_t condition)
{
...
void text::hexWordOut(outputDevice &dev, unsigned int val)
{
	...
	hexByteOut(dev, vee->u8[1]);
	hexByteOut(dev, vee->u8[0]);
...
void text::hexDWordOut(outputDevice &dev, unsigned long val)
{
	...
	hexWordOut(dev, vee->ui[1]);
	hexWordOut(dev, vee->ui[0]);
...
char * format64(const uint8_t * prgmPtr, char * strBuffer, uint8_t decimalPlaces)
{
	...
	l = tempPtr[2]->u8[6];	// load total length of binary-coded decimal bytes of converted number

	if (l == 255) strcpy_P(strBuffer, overFlowStr); // if length is 255, this number overflowed
	else
	{

		z = (char)(tempPtr[2]->u8[7]);	// load leading zero character
		y = 0;

		for (uint8_t x = 0; x < l; x++) // go through all of the binary-coded decimal bytes of converted number
		{

			b = tempPtr[2]->u8[(unsigned int)(x)];	// load a binary-coded decimal byte of number
...
char * formatDecimal(const uint8_t * prgmPtr, char * strBuffer, uint8_t windowLength, uint8_t decimalPlaces, uint8_t decimalFlag) // format number for output
{
	...
	format64(prgmPtr, strBuffer, decimalPlaces); // perform rounding of number to nearest decimal place, then format for ASCII output
...
char * doFormat(uint8_t tripIdx, uint8_t calcIdx, uint8_t windowLength, uint8_t decimalFlag)
{
	...
	if ((calcIdx < dfMaxValDisplayCount) && (tripIdx < tripSlotCount))
	{
		...
		if (calcWord == lblCGRAMtimeInSecondsIdx) format64(prgmFormatToTime, mBuff1, 0);
		else
		{
...
unsigned long convertTime(unsigned long * an)
{
	...
	copy64(tempPtr[1], (union union_64 *)(an)); // perform atomic copy of time variable to SWEET64 register 2
...
#ifdef useScreenEditor
	for (uint8_t x = 0; x < 4; x++) sfvp->u8[(unsigned int)(x)] = (uint8_t)(eepromReadVal(i++));
#else
...
#endif
#ifdef usePartialRefuel
	eepromWriteVal(pRefuelSizeIdx, 0);
#endif // usePartialRefuel
...
void doCursorUpdateSetting(void)
{
	...
	doParamReadEEPROM(screenCursor[(unsigned int)(settingScreenIdx)] + eePtrSettingsStart);
...
void doParamEditDisplay(void)
{
	...
	text::blinkStringOut(devLCD, &paramButtonChars[0], ((i == 10) && (blinkFlag)));
	text::blinkStringOut(devLCD, &paramButtonChars[4], ((i == 11) && (blinkFlag)));
...
void doGoParamEdit(void)
{
	...
	paramLength = pgm_read_byte(&paramsLength[(unsigned int)(screenCursor[(unsigned int)(settingScreenIdx)] + eePtrSettingsStart)]);

	menuLevel = paramScreenIdx;

	format64(prgmFindMaximumParamValue, mBuff2, paramLength);
	doParamFindLeft();
...
void doParamExit(void)
{
	...
	majorScreenLevelEntry(PSTR("Param Reverted"), settingScreenIdx);
...
void doParamReadMaxValue(void)
{
	...
	for (uint8_t x = 9; x < 10; x--) pBuff[(unsigned int)(x)] = mBuff2[(unsigned int)(x)];
...
void doParamReadEEPROM(uint8_t parameterPtr)
{
	...
	doParamFormatInputNumber(eepromReadVal(parameterPtr));
...
void doParamFormatInputNumber(unsigned long v)
{
	...
	init64(tempPtr[1], v);
	format64(prgmFormatToNumber, pBuff, 3);
#ifdef useLegacyLCD
	if ((menuLevel == settingScreenIdx) && ((screenCursor[(unsigned int)(settingScreenIdx)] + eePtrSettingsStart) == pContrastIdx)) LCD::setContrast((uint8_t)(v)); // adjust contrast dynamically
#endif
#ifdef useAdafruitRGBLCDmodule
	if ((menuLevel == settingScreenIdx) &&((screenCursor[(unsigned int)(settingScreenIdx)] + eePtrSettingsStart) == pLCDcolorIdx)) LCD::setRGBcolor((uint8_t)(v)); // adjust backlight color dynamically
#endif
	doParamFindLeft();
...
void doParamChangeDigit(void)
{
	...
	if (screenCursor[(unsigned int)(paramScreenIdx)] == 10) doParamWriteEEPROM();
...
#ifdef useLegacyLCD
		if ((screenCursor[(unsigned int)(settingScreenIdx)] + eePtrSettingsStart) == pContrastIdx) LCD::setContrast((uint8_t)(rformat())); // adjust contrast dynamically

#endif
#ifdef useAdafruitRGBLCDmodule
		if ((screenCursor[(unsigned int)(settingScreenIdx)] + eePtrSettingsStart) == pLCDcolorIdx) LCD::setRGBcolor((uint8_t)(rformat())); // adjust backlight color dynamically
...
void doParamWriteEEPROM(void)
{
	...
	parameterPtr = screenCursor[(unsigned int)(settingScreenIdx)] + eePtrSettingsStart;
#ifdef usePartialRefuel
	if (parameterPtr == pRefuelSizeIdx) paramChangedFlag = eepromWriteVal(pRefuelSizeIdx, rformat() + eepromReadVal(pRefuelSizeIdx));
	else paramChangedFlag = eepromWriteVal(parameterPtr, rformat());
#else // usePartialRefuel
	paramChangedFlag = eepromWriteVal(parameterPtr, rformat());
#endif // usePartialRefuel
	...
	majorScreenLevelEntry(str, settingScreenIdx);
...
void doBigTTEdisplay(void)
{
	...
	displayBigTime(format64(prgmFormatToTime, mBuff1, 3), (mainLoopHeartBeat & 0b01010101), 4);
...
void doShowSoftwareClock(void) // display software clock
{
	...
	convertTime((uint32_t *)(&clockCycles)); // perform initialization for format64
	displayBigTime(format64(prgmFormatToTime, mBuff1, 3), (mainLoopHeartBeat & 0b01010101), 4);
...
void doGoEditSoftwareClock(void)
{
	...
	convertTime((uint32_t *)(&clockCycles)); // perform initialization for format64
	format64(prgmFormatToTime, pBuff, 3); // convert system time from ticks into seconds, and format for output
...
void doEditSoftwareClockSave(void)
{
	...
	copy64(tempPtr[1], (union union_64 *)(&clockCycles)); // transfer clock value to SWEET64 register 2
	...
	copy64((union union_64 *)(&clockCycles), tempPtr[1]); // transfer SWEET64 register 2 value back to clock
...
void doCursorUpdateBarFEvS(void)
{
	...
	for (uint8_t x = 0; x < bgDataSize; x++) barGraphData[(unsigned int)(x)] = doCalculate((FEvsSpeedIdx + bgDataSize - x), calcIdx);
...
void doDisplaySystemInfo(void) // display max cpu utilization and RAM
{
	...
	text::stringOut(devLCD, format64(prgmFormatToTime, mBuff1, 3)); // output system time (since MPGuino was powered up)

	text::gotoXY(devLCD, 0, 1);
	text::stringOut(devLCD, PSTR("FREE RAM: "));
	init64(tempPtr[1], (unsigned long)(getAvailableRAMsize()));
...
void displayCPUutil(void)
{
	...
	init64(tempPtr[1], lastTimerLoopLength);
...
void doEEPROMviewDisplay(void)
{
	...
	init64(tempPtr[1], (unsigned long)(screenCursor[(unsigned int)(eepromViewIdx)]));
	text::stringOut(devLCD, format64(prgmFormatToNumber, mBuff1, 3));
	text::clearEOL(devLCD);
	text::gotoXY(devLCD, 0, 1);
	init64(tempPtr[1], eepromReadVal(screenCursor[(unsigned int)(eepromViewIdx)]));
	text::stringOut(devLCD, format64(prgmFormatToNumber, mBuff1, 3));
	text::clearEOL(devLCD);
...
void accelerationTest::goDisplay(void)
{
	...
	if (accelerationFlags & accelTestFinished) // drag function has completed - let's find out why
	{

		if (accelerationFlags & accelTestCancelled) // if this flag is set, then drag function was cancelled
		{

			msgPtr = PSTR("Drag Cancelled"); // drag race got cancelled for some reason

		}
		else // drag function finished normally
		{

			msgPtr = PSTR("Drag Finished"); // if drag distance reached, notify driver
		...
		accelerationFlags &= ~accelTestClearFlags; // clear drag race flags

		SREG = oldSREG; // restore state of interrupt flag

	}
	else // drag function in progress
	{

		accelTestStatus = (accelerationFlags ^ lastAccelTestStatus) & accelTestClearFlags; // detect any drag race flag changes

		if (accelTestStatus & accelTestActive) msgPtr = PSTR("Drag Test Start"); // if drag test started, notify driver
		if (accelTestStatus & accelTestHalfSpeed) msgPtr = PSTR("At Drag 1/2 Spd"); // if drag 1/2 speed reached, notify driver
		if (accelTestStatus & accelTestFullSpeed) msgPtr = PSTR("At Drag Speed"); // if drag full speed reached, notify driver
		if (accelTestStatus & accelTestDistance) msgPtr = PSTR("At Drag Dist"); // if drag distance reached, notify driver
	...
	lastAccelTestStatus = accelerationFlags; // update state change detector for drag function
...
void accelerationTest::goTrigger(void)
{
	...
	else
	{
		...
		if (dirty & dirtyGoodVSS) // if vehicle is still moving, then alert driver
		{
			...
			printStatusMessage(PSTR("Stop vehicle")); // vehicle is still moving - can't trigger drag mode
		...
		else // otherwise, set drag trigger
		{
			...
			if (dirty & dirtyGoodInj) // if vehicle engine is running, go trigger drag mode
			{
				...
				tripArray[(unsigned int)(dragHalfSpeedIdx)].reset(); // zero out acceleration 1/2 speed setpoint data
				tripArray[(unsigned int)(dragFullSpeedIdx)].reset(); // zero out acceleration full speed setpoint data
				tripArray[(unsigned int)(dragDistanceIdx)].reset(); // zero out acceleration distance setpoint data
				...
				lastAccelTestStatus = accelerationFlags; // initialize state change detector for drag function

				SREG = oldSREG; // restore state of interrupt flag

				printStatusMessage(PSTR("Drag Ready")); // go trigger drag mode

			}
			else
			{

				printStatusMessage(PSTR("Start Engine")); // vehicle engine has stopped - can't trigger drag mode
...
void tripSaveDisplayOut(const char * menuString)
{
	...
	text::gotoXY(devLCD, 0, 1); // go to next line
	if (guinosig == (uint8_t)(eepromReadVal(tripPtr + tripSlotSignaturePtr)))
	{

		init64(tempPtr[1], eepromReadVal(tripPtr));
		text::stringOut(devLCD, format64(prgmFormatToTime, mBuff1, 3));
...
void doTripShowDisplay(void)
{
	...
	doParamReadEEPROM(getBaseTripPointer(tripSlot) + tripVariable); // fetch value from EEPROM, and format it for decimal output
...
uint8_t doTripAutoAction(uint8_t taaMode)
{
	...
	if (eepromReadVal(pAutoSaveActiveIdx))
	{
...
void doPartialRefuelDisplay(void)
{
	...
	doParamReadEEPROM(pRefuelSizeIdx); // fetch value from EEPROM, and format it for decimal output
...
void doCursorUpdateScreenEdit(void)
{
	...
	if ((screenEditDirty & ~screenEditFlag_dirty) ^ b) // if cursor moved to a different screen function altogether
	{
		...
		c = (uint8_t)(eepromReadVal(b + eePtrScreensStart));
...
void doScreenEditDisplay(void)
{
	...
	for (uint8_t x = 0; x < 4; x++)
	{
		...
		else screenFormatValue = (uint8_t)(eepromReadVal(eePtrScreensStart + i + x));
...
void doSaveScreen(void)
{
	...
	eepromWriteVal(eePtrScreensStart + b, (unsigned long)(generateDisplayFormat(screenTripValue, screenFunctionValue)));
	screenEditDirty &= ~screenEditFlag_dirty;
...
uint8_t loadParams(void)
{
	...
	if (true)
#else
	if (eepromReadVal(pSignatureIdx) != newEEPROMsignature)
#endif
	{
		...
		t = pSignatureIdx;
		for (uint8_t x = 0; x < eePtrSettingsEnd; x++) eepromWriteVal(t++, pgm_read_dword(&params[(unsigned int)(x)]));

#ifdef useScreenEditor
		t = eePtrScreensStart;
		for (uint8_t x = 0; x < mainScreenDisplayFormatSize; x++) eepromWriteVal(t++, (unsigned long)(pgm_read_byte(&mainScreenDisplayFormats[(unsigned int)(x)])));

#endif
#ifdef useEEPROMtripStorage
		for (uint8_t x = 0; x < tripAllowedSaveSlotCount; x++) eepromWriteVal(getBaseTripPointer(x) + tripSlotSignaturePtr, 0);
...
void initGuino(void) // initialize hardware and basic system settings
{
	...
	if (eepromReadVal(pPulsesPerDistanceIdx) < 8000) VSSpause = (uint8_t)(eepromReadVal(pVSSpauseIdx)); // if pulses per distance index < 8000, it's likely that a mechanical VSS is being used
	...
	EICRx &= ~(1 << (eepromReadVal(pInjEdgeTriggerIdx) ? FI_ISC_OL : FI_ISC_CL));
	...
	for (uint8_t x = 0; x < 2; x++)
	{
		...
		analogOffset[(unsigned int)(x)] = eepromReadVal(pMAPsensorOffsetIdx + x);

	}

	volatileVariables[(unsigned int)(vFuelPressureIdx)] = eepromReadVal(pSysFuelPressureIdx); // this is in psig * 1000
	volatileVariables[(unsigned int)(vInjCorrectionIdx)] = 4096;

#endif // useChryslerMAPCorrection
	metricFlag = ((uint8_t)(eepromReadVal(pMetricModeIdx)) ? metricMode : 0);
...
void doOutputJSON(void) //skybolt added json output function
{
	...
	// first graph, fuel
	text::stringOut(devLogSerial, PSTR("{\"title\":\"fuel used "));
	doOutputNumberJSON(currentIdx, tFuelUsed, 3, PSTR(" \xfbgal\xfcL\xfd/")); // current trip fuel used
	doOutputNumberJSON(tankIdx, tFuelUsed, 3, PSTR(" \xfbgal\xfcL\xfd/")); // tank trip fuel used
	doOutputNumberJSON(instantIdx, tFuelRate, 3, PSTR(" \xfbgal\xfcL\xfd/hr\",\n")); // current rate of fuel burn in units/time
//	doOutputNumberJSON(instantIdx, tEngineSpeed, 0, PSTR(" RPM\",\n")); // rpm to test latency only vs tachometer and LCD vs raspi indicator (expect 2x looptime)
...
#ifdef trackIdleEOCdata
	doOutputNumberJSON(prgmBingoFuelRemaining, 0, 3, PSTR(" \xfbgal\xfcL\xfd e-reserve, ")); // bingo remaining fuel left
	...
	text::stringOut(devLogSerial, PSTR("{\"title\":\"distance "));
	...
	doOutputNumberJSON(prgmBingoDTE, tankIdx, 2, PSTR(" \xfbmi\xfckm\xfd e-reserve, ")); // distance to fully empty tank from bingo
	...
	// third graph, econ
	text::stringOut(devLogSerial, PSTR("{\"title\":\"fuel economy "));

	doCalculate(instantIdx, tFuelEcon); // calculate instant trip fuel economy
	copy64((union union_64 *)&instFuelEcon, tempPtr[1]); // save calculation so we don't have to calculate it again many more times

	if ((tempPtr[1]->ul[0] < 10000000) && (tempPtr[1]->ul[1] == 0)) doOutputNumberJSON((union union_64 *)&instFuelEcon, 3, PSTR(" \xfbMPG\xfcL/100km\xfd\",\n")); // instantaneous fuel economy
	else text::stringOut(devLogSerial, PSTR("infinite \xfbMPG\xfcL/100km\xfd\",\n"));

#ifdef useDragRaceFunction
	text::stringOut(devLogSerial, PSTR("\"subtitle\":\"drag stats: 0-"));
	doOutputNumberJSON(eepromReadVal(pDragSpeedIdx) / 2, 0, PSTR(" \xfbMPH\xfcKPH\xfd in "));
	doOutputNumberJSON(dragHalfSpeedIdx, tAccelTestTime, 3, PSTR(" sec, 0-")); // 0-30 time
	doOutputNumberJSON(eepromReadVal(pDragSpeedIdx), 0, PSTR(" \xfbMPH\xfcKPH\xfd in "));
	doOutputNumberJSON(dragFullSpeedIdx, tAccelTestTime, 3, PSTR(" sec, ")); // 0-60 time
	doOutputNumberJSON(dragDistanceIdx, tAccelTestTime, 3, PSTR(" sec to ")); // trap time
	doOutputNumberJSON(dragDistanceIdx, tDistance, 3, PSTR(" \xfbmi\xfckm\xfd @ ")); // trap distance
	doOutputNumberJSON(dragDistanceIdx, tSpeed, 3, PSTR(" \xfbMPH\xfcKPH\xfd, engine output ")); // trap speed
	doOutputNumberJSON(dragHalfSpeedIdx, tEstimatedEnginePower, 3, PSTR(" \xfbHP\xfckW\xfd @ ")); // estimated engine power
	doOutputNumberJSON(dragHalfSpeedIdx, tDragSpeed, 3, PSTR(" \xfbMPH\xfcKPH\xfd\",\n")); // max speed
#else // useDragRaceFunction
	text::stringOut(devLogSerial, PSTR("\"subtitle\":\"nada\",\n"));
#endif // useDragRaceFunction

	doCalculate(tankIdx, tFuelEcon); // calculate tank trip fuel economy
	copy64((union union_64 *)&tankFuelEcon, tempPtr[1]); // save calculation so we don't have to calculate it again many more times
	SWEET64(prgmJSON53tankFE, 0);
	copy64((union union_64 *)&tankFuelEcon53, tempPtr[1]); // save calculation so we don't have to calculate it again many more times

	text::stringOut(devLogSerial, PSTR("\"ranges\":["));
	doOutputNumberJSON(prgmJSON23tankFE, tankIdx, 3, PSTR(",")); // 2/3 tank fuel economy
	doOutputNumberJSON((union union_64 *)&tankFuelEcon, 3, PSTR(",")); // 3/3 tank fuel economy
	doOutputNumberJSON((union union_64 *)&tankFuelEcon53, 3, PSTR("],\n")); // 5/3 tank fuel economy

	text::stringOut(devLogSerial, PSTR("\"measures\":["));

	doCalculate(currentIdx, tFuelEcon); // calculate current fuel economy
	copy64(tempPtr[0], (union union_64 *)&tankFuelEcon53); // set up for SWEET64 version of min()
	doOutputNumberJSON(prgmJSONminValue, tankIdx, 2, PSTR(",")); // shows miles of e-reserve in bar form
//	doOutputNumberJSON((union union_64 *)&tankFuelEcon, 3, PSTR(",")); // tank fuel economy
	doOutputNumberJSON((union union_64 *)&tankFuelEcon53, 3, PSTR("],\n")); // 5/3 tank fuel economy

	text::stringOut(devLogSerial, PSTR("\"markers\":["));

	copy64(tempPtr[1], (union union_64 *)&instFuelEcon); // retrieve instant fuel economy calculated previously
	copy64(tempPtr[0], (union union_64 *)&tankFuelEcon53); // set up for SWEET64 version of min()
	doOutputNumberJSON(prgmJSONminValue, tankIdx, 2, PSTR("]}")); // shows miles of e-reserve in bar form
...
void peripheralInit(void)
{
	...
	TWI::initWrite(lcdAddress, TWI_REPEAT_START, TWI_MAIN_PROGRAM); // open TWI as master transmitter
	TWI::writeByte(MCP23017_IODIRx | MCP23017_PORTA); // specify bank A output register direction pin register address
	TWI::commitWrite(); // go write out read request

	TWI::readDevice(lcdAddress, twiMasterBufferData, 1, TWI_STOP); // become TWI master receiver, read in bank A output register direction pin register contents, and release TWI

	portSwitches = (twiMasterBufferData[0] & 0x3F) | btnShortPress21RCL;

	TWI::initWrite(lcdAddress, TWI_STOP, TWI_MAIN_PROGRAM); // open TWI as master transmitter
	TWI::writeByte(MCP23017_IODIRx | MCP23017_PORTA); // specify bank A pin direction register address
	TWI::writeByte(portSwitches); // set bank A pin register direction pins to output pins 6 and 7, input pins 0-4, and leave pin 5 alone
	TWI::writeByte(0x00); // set bank B pin register direction pins to all output
	TWI::commitWrite(); // go write out pin direction register contents

	TWI::initWrite(lcdAddress, TWI_STOP, TWI_MAIN_PROGRAM); // open TWI as master transmitter
	TWI::writeByte(MCP23017_GPPUx | MCP23017_PORTA); // specify bank A pin pull-up resistor register address
	TWI::writeByte(portSwitches); // set bank A pin pull-up resistor register pins to off for pins 6 and 7, on for pins 0-4, and leave pin 5 alone
	TWI::writeByte(0x00); // set bank B pin pull-up resistor register pins to off
	TWI::commitWrite(); // go write out pin pull-up resistor register contents

	TWI::initWrite(lcdAddress, TWI_STOP, TWI_MAIN_PROGRAM); // open TWI as master transmitter
	TWI::writeByte(MCP23017_IPOLx | MCP23017_PORTA); // specify bank A input pin polarity register address
	TWI::writeByte(portSwitches); // set bank A input pin polarity register pins to normal for pins 6 and 7, inverted for pins 0-4, and leave pin 5 alone
	TWI::writeByte(0x00); // set bank B input pin polarity register pins to normal
	TWI::commitWrite(); // go write out input pin polarity register contents

	TWI::initWrite(lcdAddress, TWI_STOP, TWI_MAIN_PROGRAM); // open TWI as master transmitter
	TWI::writeByte(MCP23017_GPINTENx | MCP23017_PORTA); // specify bank B interrupt pin enable register address
	TWI::writeByte(0x00); // disable all bank A register interrupt pins
	TWI::writeByte(0x00); // disable all bank B register interrupt pins
	TWI::commitWrite(); // go write out initialization data
...
int main(void)
{
	...
	while (true)
	{
		...
		if (timerCommand & tcTakeAnalogSample) // if main timer has commanded a sample be taken
		{
...
#endif // useCPUreading
			timerCommand &= ~tcTakeAnalogSample; // acknowledge sample command
...
#endif // useBarFuelEconVsSpeed
#ifdef useSerialDataLogging
				if (eepromReadVal(pSerialDataLoggingIdx)) doOutputDataLog();

#endif // useSerialDataLogging
#ifdef useWindowTripFilter
				if (eepromReadVal(pWindowTripFilterIdx))
				{
		...
		if (timerStatus & tsButtonRead) // see if any buttons were pressed, process related button function if so
		{
			...
			bpPtr = (buttonList *)(pgm_read_word(&(screenParameters[(unsigned int)(menuLevel)].modeButtonList)));
			bpPtr--;
			...
			if (i == buttonsUp) // if buttonpress was not in menu button list
			{
				...
				switch (j)
				{
					...
					case btnShortPressRL:
						doCursorMoveAbsolute(menuScreenIdx, 255);
						break;
...
#endif // useSavedTrips
						if (eepromReadVal(pWakeupResetCurrentOnEngineIdx)) activityFlags |= afDoResetOnVehicle;
						if (eepromReadVal(pWakeupResetCurrentOnButtonIdx)) activityFlags |= afDoResetOnButton;
